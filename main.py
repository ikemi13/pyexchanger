...
class CurrencyConverter(ttk.Frame):
    ...
    def __init__(self, parent, **args):
        ttk.Frame.__init__(
            self, parent, height=args['height'], width=args['width'])
        ...
        self.inQuantityEntry = ttk.Entry(self, font=(
            'Helvetica', 24, 'bold'), width=10, textvariable=self.__strInQuantity).place(x=38, y=23)
        self.inCurrencyCombo = ttk.Combobox(
            self, width=10, height=5, values=currency_keys, textvariable=self.inCurrency)
        self.inCurrencyCombo.place(x=38, y=71)
        self.inCurrencyCombo.bind(
            "<<ComboboxSelected>>", self.convertirDivisas)
        ttk.Label(self, text='⥮').place(x=102, y=98)
        self.outCurrencyCombo = ttk.Combobox(
            self, width=10, height=5, values=currency_keys, textvariable=self.outCurrency)
        self.outCurrencyCombo.place(x=38, y=120)
        self.outCurrencyCombo.bind(
            "<<ComboboxSelected>>", self.convertirDivisas)
        self.outQuantityLabel = ttk.Label(
            self, width=10, font=('Helvetica', 24), text='0000000000')
    ...
    def convertirDivisas(self, *args):
        ...
        if _amount != "" and _from != "" and _to != "":
            if _to == 'EUR':
                ...
                resultado = float(_amount) * \
                    currencies[_from] / currencies[_to]
...
class MainApp(Tk):
    ...
    def __init__(self):
        ...
        self.__currencyConverter = CurrencyConverter(
            self, width=378, height=229).place(x=0, y=0)
